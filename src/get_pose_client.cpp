...
private:
  ...
  void goal_response_callback(std::shared_future<GoalHandleFind::SharedPtr> future)
  {
  ...
  void result_callback(const GoalHandleFind::WrappedResult & result)
  {
    ...
    for (auto object : result.result->objects) {
      if (object.object.primitives[0].type == 1 && object.object.primitives[0].dimensions[0] < 0.05 &&
          object.object.primitives[0].dimensions[1] < 0.05 && object.object.primitives[0].dimensions[2] < 0.1) {
              RCLCPP_INFO(this->get_logger(), "X: %f", object.object.primitive_poses[0].position.x);
              RCLCPP_INFO(this->get_logger(), "Y: %f", object.object.primitive_poses[0].position.y);
          }
    }
  }
};  // class GetPoseClient
